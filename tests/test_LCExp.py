...
class test_LCExp(unittest.TestCase):
    ...
    def test_undirected(self):
        ...
        G.solver(model="L-CExp",exogenous_variables=exog,selection_variables=exog)
        ...
        self.assertTrue(G.w_ensemble_matrix.shape[1] == 10000, "test for correct second dimension of ensemble graph collection")
        self.assertTrue(np.all(G.w_ensemble_matrix >= 0.), "all matrices must be non-negative")
        tic = time.time()
        G.classification_measures(10000)
    ...
    def test_directed(self):
        ...
        G.reproduction_accuracy_s()
        toc_ra_s = time.time()- tic
        print('directed RA_s:',G.RA_s)
        
        print('time to compute ra_s',toc_ra_s,'seconds')
        self.assertTrue( isinstance(G.RA_s,np.ndarray) ,"RA_s has not the correct type")
        tic = time.time()
        G.reproduction_accuracy_w()
        print('directed RA_w:',G.RA_w)
