...
class UndirectedGraph:
    ...
    def __init__(
        ...
        if nF.is_symmetric(adjacency) == False:
            adjacency = nF.symmetrize(adjacency)
            raise TypeError("adjacency matrix has been symmetrized. If user does not want it symmetric please use DirectedGraph class!")
        ...
        self.implemented_models = ["POIS","ZIP","NB2","ZINB","L-CGeom","k-CGeom","L-IGeom","k-IGeom",
                        "k-CExp","L-CExp","L-IExp","k-IExp","L-CPareto","k-CPareto","L-CGamma","k-CGamma","L-CLognormal","k-CLognormal"]
        ...
        self.implemented_network_statistics = ["degree","annd","clust","strength","anns","clust_w"]
    ...
    def solver(
        ...
        # print('solver pre',len(self.fixed_selection_params))
        ...
        elif model in ["L-CGeom", "L-CExp", "L-CPareto",  "L-CGamma", "L-CLognormal"]:
            self.args = (self.adjacency,self.selection_variables,self.exogenous_variables,self.fixed_selection_params)
        elif model == "Logit":
        ...
        ll_fun = {
            ...
            "k-IGeom": lambda x: -lfI.ll_kIGeom_undirected(
                x, *self.args),
            "L-IExp": lambda x: -lfI.ll_LIExp(x, *self.args),
            "k-IExp": lambda x: -lfI.ll_kIExp_undirected(
                x, *self.args),
        ...
        ll_fun_binary = {
            ...
            "k-IGeom": lambda x: -lfI.ll_kIGeom_binary_undirected(
                x, *self.args),
            "L-IExp": lambda x: -lfI.ll_LIExp_binary(x, *self.args),
            "k-IExp": lambda x: -lfI.ll_kIExp_binary_undirected(
                x, *self.args),
        ...
        jac_fun = {
            ...
            "k-IGeom": lambda x: -lfI.jac_kIGeom_undirected(
                x, *self.args),
            "L-IExp": lambda x: -lfI.jac_LIExp(x, *self.args),
            "k-IExp": lambda x: -lfI.jac_kIExp_undirected(
                x, *self.args),
        ...
        if model in self.conditional_models:
            if model[:3] == "L-C":
                ...
                if len(imported_params) == 0:
                    ...
                    self.params = imported_params
                n_countries = int(np.sqrt(len(self.adjacency)))
                ll_topological = ll_fun["UBCM"](self.params[:n_countries])
                #lfC.ll_logit(self.params[:n_countries],self.adjacency,self.selection_variables,self.fixed_selection_params)
                ll_weighted = ll_fun[self.model[2:]](self.params[n_countries:])
                
                self.ll = ll_topological + ll_weighted
                self.ll_binary = ll_topological
                jac_topological = jac_fun["UBCM"](self.params[:n_countries])
                jac_weighted = jac_fun[self.model[2:]](self.params[n_countries:])
                self.jacobian = np.concatenate((jac_topological,jac_weighted))
                self.norm = np.linalg.norm(self.jacobian,ord=np.inf)
                self.aic = nF.AIC(self.ll,len(self.params))
                self.aic_binary = nF.AIC(self.ll_binary,len(self.params[:n_countries]))
                self.bic = nF.BIC(self.ll,len(self.params),len(self.adjacency))
    ...
    def gen_ensemble(self,n_ensemble = 1000):
        ...
        if self.model in self.continuous_models:
            ...
            w_mat_ensemble = eF.discrete_ensemble_matrix_undirected(self.params,self.adjacency,self.model,self.exogenous_variables,
    ...
    def classification_measures(self,n_ensemble = 1000, percentiles = (2.5,97.5),stats=[]):
        ...
        top_mat = nF.pij_matrix_undirected(self.params,self.model,self.adjacency,
